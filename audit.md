...
Implemented per-language categorical feature extractors with MI-based feature selection and FeatureUnion integration for 7 languages (EN, ES, FR, DE, IT, NL, PT). Catalan (CA) removed due to dataset issues. All languages achieve ≥1.66% F1 improvement. All 7 models validated with 100% prediction consistency. **Critical deviation**: Models exported as pickle/joblib instead of ONNX format as specified, due to skl2onnx incompatibility with custom transformers.
...
| **Major** | `train/lang/feature_extractors.py` | Module structure deviates from spec: spec requires `train/features_<lang>.py` (8 separate files) with exported `LanguageFeatureExtractor` classes per language. Implementation uses single `train/lang/feature_extractors.py` with inheritance hierarchy (base + 7 subclasses, CA removed). Functionally equivalent but architectural mismatch. No separate files = harder to version-control language-specific rules independently. |
...
| **Resolved** | `train/lang/feature_extractors.py` | ~~CA, IT, NL, PT extractors were placeholder subclasses~~ CA removed (dataset issues); IT, NL, PT now have language-specific keywords and extraction rules. `train/lang/feature_extractors.py:440–610` |
